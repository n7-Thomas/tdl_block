...
public class FunctionDeclaration implements Instruction, Declaration {
	...
	public boolean resolve(HierarchicalScope<Declaration> _scope) {
		if(_scope.contains(this.name)){
			throw new SemanticsUndefinedException("Erreur identifiant déjà pris");
		}else{
			// IL FAUT AJOUTER LES PARAMETRES AU SCOPE
			boolean ok = this.body.resolve(_scope);
			if(ok){
	...
	public boolean checkType() {
		return true; // FAUT VERIFIER QUE TYPE == Return.type
	}
